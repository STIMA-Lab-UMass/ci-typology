...
class OpenAIResNonRes4Classifier():
    ...
    def create_manual_res(self, batch_data, classification_type):
        ...
        if classification_type == 'naics':
            ans_res = [
                {"index": item['index'], "naics_code": None, "category": "Others"}
                for item in batch_dic
            ]
        elif classification_type == 'res_non_res':
            ans_res = [
                {"index": item['index'], "is_residential": False}
                for item in batch_dic
            ]
        return ans_res

    def api_req_prompt(self, prompt, batch_size, batch_data, retries=3, delay=5, classification_type = 'naics'):
        ...
        while attempt < retries:
            try:
                ...
                response = self.client.chat.completions.create(
                    model=model_name,
                    messages=[
                        {"role": "system", "content": "You are a text classifier. Help classify the given data."},
        ...
        return self.create_manual_res(batch_data, classification_type)


    def get_naics_prompt(self, batch_data):
    ...
    def get_res_non_res_prompt(self, batch_data):

        batch_dic = batch_data.to_dict(orient='records')
        prompt = f"""
        Classify the following data into residential and non-residential category.

        A residential building includes apartments, condominiums, farmhouses, single- or multi-family homes, townhouses. If the data is residential
        type return true for "is_residential", otherwise return false.

        Ensure the response is in JSON format as a list of each row, in the following structure ("index" should be the value from index column of the data):
        [
            {{"index": batch_dic['index'], "is_residential": true/false}},
            ...
        ]
    ...
    def parse_res_non_res_response(self, data, res_data):
        processed_rows = []
        expected_columns = list(data.columns)
        if 'is_residential' not in expected_columns:
            expected_columns.append('is_residential')
        for classification_result in res_data:
            index = int(classification_result["index"])
            is_residential = classification_result["is_residential"]

            retry_count = 0
            max_retries = 3

            while not isinstance(is_residential, bool) and retry_count < max_retries:
                retry_count += 1
                time.sleep(3)
                single_row = data[data['index'] == index].reset_index(drop=True)
                if single_row.empty:
                    break
                prompt = self.get_res_non_res_prompt(single_row)
                response = self.api_req_prompt(prompt, 1, single_row, classification_type='res_non_res')
                try:
                    index = int(response[0]["index"])
                    is_residential = response[0]["is_residential"]
            ...
            if row.empty:
                continue
            row = row.iloc[0].copy()
            row['is_residential'] = is_residential
        ...
        if 'is_residential' not in final_df.columns:
            final_df['is_residential'] = False
        return final_df
    ...
    def _classify_batch(self, batch_data):
        res_non_res_prompt = self.get_res_non_res_prompt(batch_data)
        response = self.api_req_prompt(res_non_res_prompt, batch_data.shape[0], batch_data, classification_type='res_non_res')

        res_non_res_data = self.parse_res_non_res_response(batch_data, response)
        if res_non_res_data.empty:
            return res_non_res_data

        if 'classification' not in res_non_res_data.columns:
            res_non_res_data['classification'] = None
        if 'naics_code' not in res_non_res_data.columns:
            res_non_res_data['naics_code'] = None

        non_res_data = res_non_res_data[res_non_res_data['is_residential'] == False].reset_index(drop=False)

        if not non_res_data.empty:
            prompt = self.get_naics_prompt(non_res_data)
            res_json = self.api_req_prompt(prompt, non_res_data.shape[0], non_res_data)
            ...
            for classification_result in res_json:
                ...
                while self.check_not_valid(code, classification) and retry_count < max_retries:
                    ...
                    if classification in self.categories:
                        ...
                        single_row = res_non_res_data[res_non_res_data['index'] == index].reset_index(drop=True)
                        if single_row.empty:
                            break
                        prompt = self.get_naics_prompt(single_row)
                ...
                mask = res_non_res_data['index'] == index
                res_non_res_data.loc[mask, "classification"] = classification
                res_non_res_data.loc[mask, "naics_code"] = str(code)

        if 'index' in res_non_res_data.columns:
            res_non_res_data = res_non_res_data.drop(columns=['index'])
    ...
    def openai_classifier(self, admin_filter=None):
        ...
        logging.info(f"Running OpenAI Res/Non-Res classification with {worker_count} parallel worker(s).")

        def _classify_single(data_type):
            logging.info(f'Starting classification for {data_type}.')
...
if __name__ == '__main__':
    ...
    openai_classifier = OpenAIResNonRes4Classifier(country_code, index, args.data_types)
